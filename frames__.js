(function(window, snch) {
  ...
  var fn = MainLoopJS.prototype = {};

  fn.constructor = MainLoopJS;

  fn.setup = function setup() {
    this.now = Date.now();
    this.then = Date.now();
...
(function(window, snch) {
  ...
  var fn = PreloaderJS.prototype = {};

  fn.constructor = PreloaderJS;

  fn.setup = function init() {
    this.load();
  };

  fn.load = function load() {
    var img = new Image(),
      ref = this;

    img.src = this.queue[indice];
    img.onload = function() {
      ref.done(img);
    };
  };

  fn.done = function done(img) {
    indice += 1;
    dependencies.push(img);
    if (indice < this.queue.length) {
      this.load();
    } else {
      this.onComplete(dependencies);
    }
  };
...
Engine.prototype = {
  ...
  backward: function backward() {
    console.log(this.tileIndice, this.spriteIndice,this.spriteSettings[this.spriteIndice].tiles);
    this.tileIndice--;
    if (this.spriteSettings[this.spriteIndice].tiles <= 0) {
      if (this.spriteIndice <= 0) {
        if (this.options.loop) {
          this.reversing = false;
          this.reset();
        } else {
      ...
      this.spriteIndice--;
      this.tileIndice = this.spriteSettings[this.spriteIndice].tiles;
  ...
  setTile: function setTile() {
    ...
    console.log(this.tileColunm, this.tileLine);
